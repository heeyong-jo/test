...
function applyAppendixFont() {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    if (el) el.style.fontSize = appendixFontSize + 'px';
  }
}
...
function initBibleBooks() {
  ...
  // 이미 렌더링되었으면 다시 하지 않음
  if (otDiv.innerHTML && otDiv.innerHTML.includes('창')) return;
...
async function selectBook(bookName) {
  ...
  if (selectedBookTitle) selectedBookTitle.innerHTML = '📖 ' + currentBookInfo.name;
...
async function loadChapter(chapter) {
  ...
  if (verseTitle) verseTitle.innerHTML = '📖 ' + currentBookInfo.name + ' ' + currentChapter + '장';
  if (verseContent) {
  ...
  try {
    ...
    const keys = [currentBookInfo.name, currentBookInfo.abbr];
    for (let i = 0; i < keys.length; i++) {
      if (data[keys[i]] && data[keys[i]][currentChapter]) {
      ...
      if (bookData && bookData[currentChapter]) {
        ...
        for (let i = 0; i < verseNumbers.length; i++) {
          ...
          html += '<div style="display:flex;gap:12px;padding:10px 0;border-bottom:1px solid #eee5f8;">' +
            ...
            '<div style="font-size:' + fontSize + 'px;line-height:1.6;flex:1;">' + escapeHtml(text) + '</div>' +
          '</div>';
        }
        verseContent.innerHTML = html;
      } else {
        verseContent.innerHTML = '<div style="text-align:center;padding:40px;color:#b91c1c;">📖 ' + currentBookInfo.name + ' ' + currentChapter + '장 데이터가 없습니다</div>';
      }
    }
  } catch (e) {
  ...
  if (hymnSearchQuery) {
    ...
    for (let i = 1; i <= 645; i++) {
      ...
      if (isNum) {
        ...
        match = (hymnTitles[String(i)] || '').toLowerCase().includes(q);
      }
...
function openHymn(no) {
  ...
  if (!modal) {
    ...
    prevBtn.onclick = function() { openHymn(currentHymnNo - 1); };
    ...
    nextBtn.onclick = function() { openHymn(currentHymnNo + 1); };
...
function changeBibleReadingFontSize(delta) {
  ...
  if (content) content.style.fontSize = bibleReadingFontSize + 'px';
}
