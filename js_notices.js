  ...
  if (photosDiv) {
    if (notice.photos && notice.photos.length) {
      photosDiv.innerHTML = notice.photos.map(src => 
        `<img src="${src}" style="width:100%;border-radius:12px;margin-bottom:8px;cursor:pointer;" onclick="window.open(this.src)">`
      ).join('');
    } else {
      photosDiv.innerHTML = '';
    }
...
function showAllNotices() {
  ...
  if (listDiv) {
    ...
    notices.forEach((notice, i) => {
      html += `
        <div style="padding:12px;border-bottom:1px solid #eee;cursor:pointer;" onclick="viewNotice('${notice.id}'); document.getElementById('all-notices-modal').style.display='none';">
