...
window.addEventListener('load', () => {
    ...
    if (window.FB_READY && window.FB) {
      ...
      try {
        ...
        if (churchSnap.exists()) {
          ...
          if (churchData.greeting && typeof renderGreeting === 'function') renderGreeting(churchData.greeting);
          if (churchData.history && typeof renderHistory === 'function') renderHistory(churchData.history);
        }
      } catch(e) {
  ...
  setTimeout(() => {
    ...
    if (logged && (Date.now() - logged.ts < 86400000 * 7)) {
      const admin = (typeof ADMIN_ACCOUNTS !== 'undefined') ? ADMIN_ACCOUNTS.find(a => a.id === logged.id) : null;
